...
const AttachmentsStoreCtx =
  React.createContext<StoreApi<AttachmentsCtxType> | null>(null);

export function useAttachments(): AttachmentsCtxType;
export function useAttachments<T>(
  selector: (state: AttachmentsCtxType) => T,
): T;
...
const AttachmentStoreCtx =
  React.createContext<StoreApi<AttachmentCtxType> | null>(null);
...
export function useAttachment<T>(selector?: (state: AttachmentCtxType) => T) {
  ...
  if (!store)
    throw new Error(
      "useAttachment must be used within an AttachmentByIndexProvider.",
    );
  return useStore(store, selector as any);
}
