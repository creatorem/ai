...
const SuggestionsStoreCtx =
  React.createContext<StoreApi<SuggestionsCtxType> | null>(null);

export function useSuggestions(): SuggestionsCtxType;
export function useSuggestions<T>(
  selector: (state: SuggestionsCtxType) => T,
): T;
...
const SuggestionStoreCtx =
  React.createContext<StoreApi<SuggestionState> | null>(null);
...
export function useSuggestion<T>(selector?: (state: SuggestionState) => T) {
  ...
  if (!store)
    throw new Error(
      "useSuggestion must be used within a SuggestionByIndexProvider.",
    );
  return useStore(store, selector as any);
}
