...
import {
  emptyStorageThreadAdapter,
  saveThread,
  deleteThread as deleteStoredThread,
  archiveThread as archiveStoredThread,
  unarchiveThread as unarchiveStoredThread,
  renameThread as renameStoredThread,
} from "../../adapters/empty-storage-adapter";
...
const ThreadListStoreCtx =
  React.createContext<StoreApi<ThreadListCtxType> | null>(null);
...
export function useThreadList<T>(selector?: (state: ThreadListCtxType) => T) {
  ...
  if (!store)
    throw new Error("This component must be used within ThreadListProvider.");
...
export function ThreadListProvider({
  ...
  const deleteThread = useCallback(
    (threadId: string) => {
      ...
      threadsStore
        .getState()
        .setThreadIds((prev) => prev.filter((id) => id !== threadId));
      threadsStore
        .getState()
  ...
  const archiveThread = useCallback(
    (threadId: string) => {
      ...
      threadsStore
        .getState()
        .setArchivedThreadIds((prev) => [threadId, ...prev]);
    },
    [threadsStore],
  );
