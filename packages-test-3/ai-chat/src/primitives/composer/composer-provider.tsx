...
const ComposerStoreCtx = React.createContext<StoreApi<ComposerCtxType> | null>(
  null,
);

// Fallback store for optional usage (avoids conditional hook calls)
const _FALLBACK_STORE = createStore<ComposerCtxType>(
  () => ({}) as ComposerCtxType,
);
...
export function useComposer<T>(
  ...
  const value = useStore(
    ...
    isSelector
      ? (selectorOrOptions as (state: ComposerCtxType) => T)
      : (undefined as any),
  );
...
export function ComposerPrimitiveRoot({
  children,
  id,
  defaultText = "",
  editMessageId,
  ...
  const [attachmentAccept, setAttachmentAccept] =
    useState<Composer["attachmentAccept"]>("*");
  const [type, setType] = useState<Composer["type"]>("thread");
  const [dictation, setDictation] = useState<DictationState | undefined>(
    undefined,
  );

  // Dictation internal refs (mutable state that doesn't trigger re-renders)
  const _dictationSessionRef = useRef<DictationAdapter.Session | undefined>(
    undefined,
  );
  ...
  const _isActiveSession = useCallback(
    (sessionId: number, session: DictationAdapter.Session): boolean => {
      return (
  ...
  const _cleanupDictation = useCallback(
    (options?: { sessionId: number | undefined }) => {
  ...
  const addAttachment = useCallback(
    async (newAttachment: Omit<Attachment, "id">): Promise<void> => {
  ...
  const startDictation = useCallback((): void => {
    ...
    const unsubSpeech = session.onSpeech((result) => {
      ...
      if (isFinal) {
        ...
        setTextState(
          _dictationBaseTextRef.current + _currentInterimTextRef.current,
        );
        setDictation((prev) => {
          if (prev) {
  ...
  return (
    <ComposerStoreCtx.Provider value={storeRef.current}>
      {children}
