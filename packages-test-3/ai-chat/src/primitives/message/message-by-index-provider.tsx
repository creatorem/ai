...
const MessageStoreCtx = React.createContext<StoreApi<MessageCtxType> | null>(
  null,
);
...
export function MessageByIndexProvider({
  ...
  const parentId = useThread((s) =>
    index > 0 ? (s.messages[index - 1]?.id ?? null) : null,
  );
  ...
  const metadata = {
    submittedFeedback: (message.metadata as Record<string, unknown> | undefined)
      ?.submittedFeedback as
  ...
  if (storeRef.current === null) {
    storeRef.current = createStore<MessageCtxType>(() => ({
      ...
      switchToBranch: (options: {
        ...
        const currentBranches = state.getBranches(
          storeRef.current!.getState().id,
        );
        const currentIndex = currentBranches.indexOf(
          storeRef.current!.getState().id,
        );
        if (options.position === "previous" && currentIndex > 0) {
          ...
          currentIndex < currentBranches.length - 1
        ) {
          state.switchToBranch(currentBranches[currentIndex + 1]!);
        }
      },
  ...
  return (
    <MessageStoreCtx.Provider value={storeRef.current}>
      {children}
