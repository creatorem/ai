...
export type ThreadCtxType = Thread &
  Omit<
    ReturnType<typeof useChat<Thread["messages"][0]>>,
    "status" | "setMessages" | "sendMessage"
  > & {
    ...
    .map((part) => {
      ...
      if (isToolUIPart(part)) {
        ...
        return {
          ...
          result,
          isError,
          ...((part as any).artifact !== undefined
            ? { artifact: (part as any).artifact }
            : {}),
        };
      }
...
const ThreadStoreCtx = React.createContext<StoreApi<ThreadCtxType> | null>(
  null,
);
...
export function useThread<T>(selector?: (state: ThreadCtxType) => T) {
  ...
  if (!store)
    throw new Error("This component must be used within ThreadCtx.Provider.");
...
export function ThreadPrimitiveRoot({
  children,
  ...value
}: {
  ...
  const transportRef = useRef<DefaultChatTransport<
    Thread["messages"][0]
  > | null>(null);
      ...
      prepareSendMessagesRequest: async (options) => {
        ...
        return {
          ...
          body: {
            ...
            [BODY_KEY.DISABLED_TOOLS]: Object.entries(ctx.tools)
              .map(([, t]) => (t.disabled ? t.toolName : null))
              .filter((t) => t !== null),
  ...
  const switchToBranchFn = useCallback(
      ...
      if (activeThreadId) {
        ...
        if (threadAdapter.generateTitle) {
          ...
          const cleanContent =
            ...
            typeof firstMessage.parts[0] === "object" &&
            "text" in firstMessage.parts[0]
              ? (firstMessage.parts[0] as any).text
              : "") ||
            "";
  ...
  return (
    <ThreadStoreCtx.Provider value={storeRef.current}>
      <AttachmentsProvider
        attachments={[]}
