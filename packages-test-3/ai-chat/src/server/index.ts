...
export const frontendTools = (body: Record<string, unknown>): Record<string, Tool<any, any>> => {
    const tools = z.record(z.string(), z.any()).nullable().optional().safeParse(body?.[BODY_KEY.TOOLS]).data || {};
...
export const getServerUtils = (body: Record<string, unknown>) => {
    return {
        getModel: (resolveModel: ModelResolver = defaultModelResolver) => getModel(body, resolveModel),
