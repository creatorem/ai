...
const useMessageViewportRef = () => {
  ...
  const setUserMessageHeight = useThreadViewport((s) => s.setUserMessageHeight);
 ...
 * ```
 */
export const AiChatWebProvider = ({
  children,
  components,
  hooks,
  fn,
  ...
  return (
    <RuntimeProvider
      ...
      components={{ ...webComponents, ...components }}
      functions={{ ...webFunctions, ...fn }}
    >
      {children}
    </RuntimeProvider>
  );
