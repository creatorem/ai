...
import { ThreadState } from "../types/entities/thread";
import { useAiChat, useAiChatShallow } from "../store";
import {
  useExternalMessageConverter,
  convertExternalMessages,
} from "./external-message-converter";
import { getExternalStoreMessages } from "./get-external-store-message";
...
export const createMessageConverter = <T extends object>(
  ...
  const result = {
    ...
    useOriginalMessages: () => {
      const partMessages = useAiChatShallow((s) => {
        if (s.part) return getExternalStoreMessages(s.part);
        return undefined;
      });

      const messageMessages = useAiChat<T[]>(({ message }) =>
...
// Export as unstable_createMessageConverter for backwards compatibility
export { createMessageConverter as unstable_createMessageConverter };
